...
Time::DefaultCUC timestamps1[6] = {Time::DefaultCUC(2), Time::DefaultCUC(4), Time::DefaultCUC(5), Time::DefaultCUC(7), Time::DefaultCUC(9),
                                   Time::DefaultCUC(11)};
Time::DefaultCUC timestamps2[5] = {Time::DefaultCUC(0), Time::DefaultCUC(1), Time::DefaultCUC(4), Time::DefaultCUC(15), Time::DefaultCUC(22)};
Time::DefaultCUC timestamps3[4] = {Time::DefaultCUC(4), Time::DefaultCUC(7), Time::DefaultCUC(9), Time::DefaultCUC(14)};
Time::DefaultCUC timestamps4[8] = {Time::DefaultCUC(4), Time::DefaultCUC(6), Time::DefaultCUC(34), Time::DefaultCUC(40), Time::DefaultCUC(44),
                                   Time::DefaultCUC(51), Time::DefaultCUC(52), Time::DefaultCUC(58)};
...
void invalidPacketStoreCreationRequest(Message& request) {
	NumOfPacketStores numOfPacketStores = 5;
	request.appendUint16(numOfPacketStores);
...
TEST_CASE("Deleting packet stores") {
	...
	SECTION("Invalid deletion of specified packet stores") {
		...
		storageAndRetrieval.getPacketStore(packetStoreIds[1]).byTimeRangeRetrievalStatusEnabled = true;
	...
	SECTION("Both valid and invalid deletion requests") {
		...
		Message request(StorageAndRetrievalService::ServiceType,
		                StorageAndRetrievalService::MessageType::DeletePacketStores, Message::TC, 1);
		NumOfPacketStores numOfPacketStores = 8;
		request.appendUint16(numOfPacketStores);
		...
		storageAndRetrieval.getPacketStore(correctPacketStoreIds[1]).byTimeRangeRetrievalStatusEnabled = true;
...
TEST_CASE("Changing the open retrieval start-time-tag") {
	SECTION("Successful change of the start-time-tag") {
		...
		Time::DefaultCUC startTimeTag(200);
		NumOfPacketStores numOfPacketStores = 2;
		request.append<Time::DefaultCUC>(startTimeTag);
		request.appendUint16(numOfPacketStores);
...
TEST_CASE("Resuming the open retrieval process") {
	...
	SECTION("Failed resuming of the open retrieval") {
		...
		for (int i = 0; i < numOfPacketStores / 2; i++) {
			auto packetStoreId = correctPacketStoreIds[i];
			storageAndRetrieval.getPacketStore(packetStoreId).byTimeRangeRetrievalStatusEnabled = true;
			storageAndRetrieval.getPacketStore(packetStoreId).openRetrievalStatus = PacketStore::Suspended;
			request.appendString(packetStoreId);
		}
	...
	SECTION("Both successful and failed attempts to resume the open retrieval of all packet stores") {
		...
		storageAndRetrieval.getPacketStore(packetStoreIds[2]).byTimeRangeRetrievalStatusEnabled = true;
		storageAndRetrieval.getPacketStore(packetStoreIds[3]).byTimeRangeRetrievalStatusEnabled = true;
...
TEST_CASE("Starting the by-time-range retrieval of packet stores") {
	...
	SECTION("Failed starting of the by-time-range retrieval") {
		...
		for (int i = 0; i < numOfPacketStores / 2; i++) {
			...
			storageAndRetrieval.getPacketStore(packetStoreId).byTimeRangeRetrievalStatusEnabled = i % 2 == 0;

			request.appendString(packetStoreId);
			Time::DefaultCUC timeTag1(20);
			Time::DefaultCUC timeTag2(40);
			request.append<Time::DefaultCUC>(timeTag1);
			request.append<Time::DefaultCUC>(timeTag2);
		}
		storageAndRetrieval.getPacketStore(correctPacketStoreIds[3]).byTimeRangeRetrievalStatusEnabled = false;
		...
		for (int i = 0; i < numOfPacketStores / 2; i++) {
			...
			REQUIRE(packetStore.byTimeRangeRetrievalStatusEnabled == (i % 2 == 0));
			REQUIRE(packetStore.retrievalStartTime == Time::DefaultCUC(0));
			REQUIRE(packetStore.retrievalEndTime == Time::DefaultCUC(0));
		}
		REQUIRE(storageAndRetrieval.getPacketStore(correctPacketStoreIds[3]).byTimeRangeRetrievalStatusEnabled == false);
	...
	SECTION("Invalid window requested") {
		...
		REQUIRE(storageAndRetrieval.getPacketStore(packetStoreIds[2]).byTimeRangeRetrievalStatusEnabled == false);
		REQUIRE(storageAndRetrieval.getPacketStore(packetStoreIds[3]).byTimeRangeRetrievalStatusEnabled == false);
...
TEST_CASE("Aborting the by-time-range retrieval of packet stores") {
	SECTION("Successful aborting of the by-time-range retrieval") {
		...
		CHECK(ServiceTests::count() == 0);
		for (int i = 0; i < numOfPacketStores; i++) {
			REQUIRE(storageAndRetrieval.getPacketStore(packetStoreIds[i]).byTimeRangeRetrievalStatusEnabled == false);
		}
		REQUIRE(storageAndRetrieval.getPacketStore(packetStoreIds[3]).byTimeRangeRetrievalStatusEnabled == true);
...
TEST_CASE("Reporting the status of packet stores") {
	SECTION("Valid reporting of the packet store status") {
		...
		for (auto& packetStoreId: packetStoreIds) {
			...
			packetStore.byTimeRangeRetrievalStatusEnabled = (count % 2 != 0);
...
TEST_CASE("Resizing the packet stores") {
	...
	SECTION("Failed resizing of packet stores") {
		...
		for (auto& packetStoreId: packetStoreIds) {
			...
			packetStore.byTimeRangeRetrievalStatusEnabled = (index == 1);
...
TEST_CASE("Changing the packet store type to circular") {
	...
	SECTION("Failed changing of type to circular") {
		...
		for (auto& packetStoreId: correctPacketStoreIds) {
			...
			packetStore.byTimeRangeRetrievalStatusEnabled = (count == 1);
...
TEST_CASE("Changing the packet store type to bounded") {
	...
	SECTION("Failed changing of type to bounded") {
		...
		for (auto& packetStoreId: correctPacketStoreIds) {
			...
			storageAndRetrieval.getPacketStore(packetStoreId).byTimeRangeRetrievalStatusEnabled = (count == 1);
			storageAndRetrieval.getPacketStore(packetStoreId).openRetrievalStatus =
...
TEST_CASE("Changing the virtual channel of packet stores") {
	...
	SECTION("Failed change of virtual channel") {
		...
		int count = 0;
		for (auto& packetStoreId: correctPacketStoreIds) {
			storageAndRetrieval.getPacketStore(packetStoreId).byTimeRangeRetrievalStatusEnabled = (count == 0);
			storageAndRetrieval.getPacketStore(packetStoreId).openRetrievalStatus =
...
TEST_CASE("Reporting the content summary of packet stores") {
	SECTION("Successful content summary report of specified packet stores") {
		...
		CHECK(report.read<Time::DefaultCUC>() == timestamps1[0]);
		CHECK(report.read<Time::DefaultCUC>() == timestamps1[5]);
		CHECK(report.readUint32() == Time::DefaultCUC(5).formatAsBytes());
		...
		CHECK(report.read<Time::DefaultCUC>() == timestamps2[0]);
		CHECK(report.read<Time::DefaultCUC>() == timestamps2[4]);
		CHECK(report.readUint32() == Time::DefaultCUC(5).formatAsBytes());
	...
	SECTION("Successful content summary report of all packet stores") {
		...
		CHECK(report.read<Time::DefaultCUC>() == timestamps1[0]);
		CHECK(report.read<Time::DefaultCUC>() == timestamps1[5]);
		CHECK(report.readUint32() == Time::DefaultCUC(15).formatAsBytes());
		CHECK(report.read<PercentageFilled>() == 60);
		...
		CHECK(report.read<Time::DefaultCUC>() == timestamps4[0]);
		CHECK(report.read<Time::DefaultCUC>() == timestamps4[7]);
		CHECK(report.readUint32() == Time::DefaultCUC(20).formatAsBytes());
		CHECK(report.read<PercentageFilled>() == 80);
		CHECK(report.read<PercentageFilled>() == 60);
		// Packet store 4
		report.readString(data, ECSSPacketStoreIdSize);
		CHECK(std::equal(std::begin(packetStoreData4), std::end(packetStoreData4), std::begin(data)));
		CHECK(report.read<Time::DefaultCUC>() == timestamps3[0]);
		CHECK(report.read<Time::DefaultCUC>() == timestamps3[3]);
		CHECK(report.readUint32() == Time::DefaultCUC(15).formatAsBytes());
		CHECK(report.read<PercentageFilled>() == 40);
		CHECK(report.read<PercentageFilled>() == 0);
...
TEST_CASE("Deleting packet store content") {
	SECTION("Successful deletion of content, specified time-tag in the middle of the packets stored") {
		...
		Time::DefaultCUC storageTime(5);
		NumOfPacketStores numOfPacketStores = 2;
		request.append<Time::DefaultCUC>(storageTime);
		request.appendUint16(numOfPacketStores);
		...
		Time::DefaultCUC expectedTimeStamps1[3] = {Time::DefaultCUC(7), Time::DefaultCUC(9), Time::DefaultCUC(11)};
		Time::DefaultCUC expectedTimeStamps2[2] = {Time::DefaultCUC(15), Time::DefaultCUC(22)};
		Time::DefaultCUC leftTimeStamps1[3];
	...
	SECTION("Successful deletion of content, specified time-tag is smaller than the min stored timestamp") {
		...
		Time::DefaultCUC expectedTimeStamps1[4] = {Time::DefaultCUC(4), Time::DefaultCUC(7), Time::DefaultCUC(9), Time::DefaultCUC(14)};
		Time::DefaultCUC expectedTimeStamps2[8] = {Time::DefaultCUC(4), Time::DefaultCUC(6), Time::DefaultCUC(34), Time::DefaultCUC(40),
		                                           Time::DefaultCUC(44),
		                                           Time::DefaultCUC(51), Time::DefaultCUC(52), Time::DefaultCUC(58)};
		Time::DefaultCUC leftTimeStamps1[4];
	...
	SECTION("Successful deletion of content, specified time-tag is larger than the max stored timestamp") {
		...
		Time::DefaultCUC storageTime(59);
		NumOfPacketStores numOfPacketStores = 2;
		request.append<Time::DefaultCUC>(storageTime);
		request.appendUint16(numOfPacketStores);
	...
	SECTION("Both successful and failed deletion of content for all packet stores") {
		...
		Time::DefaultCUC storageTime(15);
		NumOfPacketStores numOfPacketStores = 0;
		request.append<Time::DefaultCUC>(storageTime);
		request.appendUint16(numOfPacketStores);
		...
		Time::DefaultCUC expectedTimeStamps4[6] = {Time::DefaultCUC(34), Time::DefaultCUC(40), Time::DefaultCUC(44), Time::DefaultCUC(51),
		                                           Time::DefaultCUC(52), Time::DefaultCUC(58)};

		Time::DefaultCUC leftTimeStamps1[6];
		Time::DefaultCUC leftTimeStamps2[5];
	...
	SECTION("Failed deletion of content") {
		...
		for (int i = 3; i < 7; i++) {
			auto packetStoreId = finalIds[i];
			storageAndRetrieval.getPacketStore(packetStoreId).byTimeRangeRetrievalStatusEnabled = (i == 4 || i == 6);
			storageAndRetrieval.getPacketStore(packetStoreId).openRetrievalStatus =
...
TEST_CASE("Copying packets in time window, from tag to tag") {
	SECTION("Both time-tags earlier than the earliest time-tag") {
		...
		Time::DefaultCUC timeTag1(0);
		Time::DefaultCUC timeTag2(1);
	...
	SECTION("Both time-tags in between the stored timestamps") {
		...
		uint8_t typeOfTimeWindow = StorageAndRetrievalService::TimeWindowType::FromTagToTag;
		Time::DefaultCUC timeTag1(35);
		Time::DefaultCUC timeTag2(52);
	...
	SECTION("Time-tag1 in between the stored Time::DefaultCUC and tag2 larger than the max timestamp") {
		/**
		 * CASE 3:
		 *
		...
		uint8_t typeOfTimeWindow = StorageAndRetrievalService::TimeWindowType::FromTagToTag;
		Time::DefaultCUC timeTag1(3);
		Time::DefaultCUC timeTag2(27);
	...
	SECTION("Both time-tags larger than largest stored time-tag") {
		...
		uint8_t typeOfTimeWindow = StorageAndRetrievalService::TimeWindowType::FromTagToTag;
		Time::DefaultCUC timeTag1(12);
		Time::DefaultCUC timeTag2(14);
		auto fromPacketStoreId = packetStoreIds[0];
		auto toPacketStoreId = packetStoreIds[2];
	...
	SECTION("Invalid time window requested") {
		...
		uint8_t typeOfTimeWindow = StorageAndRetrievalService::TimeWindowType::FromTagToTag;
		Time::DefaultCUC timeTag1(26);
		Time::DefaultCUC timeTag2(17);
		auto fromPacketStoreId = packetStoreIds[0];
		auto toPacketStoreId = packetStoreIds[2];
