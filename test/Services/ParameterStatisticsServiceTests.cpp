...
TEST_CASE("Reporting of statistics") {
	SECTION("Report statistics, with auto statistic reset disabled with TC") {
		...
		CHECK(report.read<TimeStamps>().formatAsBytes() == 86769000); // start time
		CHECK(report.read<TimeStamps>().formatAsBytes() == 86769000); // end time
		...
		CHECK(report.read<TimeStamps>().formatAsBytes() == 86769000); // max time
		CHECK(report.readFloat() == 3);         // min value
		CHECK(report.read<TimeStamps>().formatAsBytes() == 86769000); // min time
