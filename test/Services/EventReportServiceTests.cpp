...
TEST_CASE("Informative Event Report TM[5,1]", "[service][st05]") {
	...
	// Checks for the data-members of the report ECSSMessage created
	...
	CHECK(report.packetType == ECSSMessage::TM); // packet type(TM = 0, TC = 1)
...
TEST_CASE("Enable Report Generation TC[5,5]", "[service][st05]") {
	...
	ECSSMessage message(EventReportService::ServiceType, EventReportService::MessageType::EnableReportGenerationOfEvents, ECSSMessage::TC, 1);
...
TEST_CASE("Disable Report Generation TC[5,6]", "[service][st05]") {
	...
	ECSSMessage message(EventReportService::ServiceType, EventReportService::MessageType::DisableReportGenerationOfEvents, ECSSMessage::TC, 1);
	message.appendUint16(2);
	message.appendEnum16(eventID[0]);
	message.appendEnum16(eventID[1]);
	MessageParser::execute(message);
...
TEST_CASE("Request list of disabled events TC[5,7]", "[service][st05]") {
	ECSSMessage message(EventReportService::ServiceType, EventReportService::MessageType::ReportListOfDisabledEvents,
	                    ECSSMessage::TC, 1);
	MessageParser::execute(message);
	REQUIRE(ServiceTests::hasOneMessage());
...
TEST_CASE("List of Disabled Events Report TM[5,8]", "[service][st05]") {
	...
	ECSSMessage report = ServiceTests::get(0);
	// Check for the data-members of the report ECSSMessage created
	CHECK(report.serviceType == EventReportService::ServiceType);
	CHECK(report.messageType == EventReportService::MessageType::DisabledListEventReport);
	CHECK(report.packetType == ECSSMessage::TM); // packet type(TM = 0, TC = 1)
