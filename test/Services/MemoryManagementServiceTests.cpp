...
TEST_CASE("TC[6,1] Load Object Memory Data", "[service][st06]") {
	...
	SECTION("Multiple instruction load") {
		...
		const uint8_t testData1[] = "First block";
		const FileDataLength dataLength1 = sizeof(testData1);
		
		request.append<Offset>(offset1);
		request.append<FileDataLength>(dataLength1);
		request.appendString(String<ECSSMaxFixedOctetStringSize>(testData1, dataLength1));

		// Second data block
		const Offset offset2 = 100;
		const uint8_t testData2[] = "Second block";
		const FileDataLength dataLength2 = sizeof(testData2);
		
		request.append<Offset>(offset2);
		request.append<FileDataLength>(dataLength2);
		request.appendString(String<ECSSMaxFixedOctetStringSize>(testData2, dataLength2));
