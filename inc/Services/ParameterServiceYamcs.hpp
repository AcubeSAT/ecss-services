#ifndef ECSS_SERVICES_PARAMETERSERVICEYAMCS_HPP
#define ECSS_SERVICES_PARAMETERSERVICEYAMCS_HPP
 ...
 */
class ParameterServiceYamcs : public Service {
private:
	 ...
	 */
	void initializeParameterMapYamcs();
...
public:
	 ...
	 */
	ParameterServiceYamcs() {
		serviceType = ServiceType;
		initializeParameterMapYamcs();
	}
	...
	bool parameterExists(uint16_t parameterId) const {
		return parametersYamcs.find(parameterId) != parametersYamcs.end();
	}
	...
	std::optional<std::reference_wrapper<ParameterBase>> getParameter(uint16_t parameterId) const {
		auto parameter = parametersYamcs.find(parameterId);

		if (parameter != parametersYamcs.end()) {
...
#endif //ECSS_SERVICES_PARAMETERSERVICEYAMCS_HPP
