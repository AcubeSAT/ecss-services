...
private:
	 ...
	 *
	 * If the open retrieval of the packet store is in progress, the newly stored packet is also released to the
	 * ground station, as per requirement 6.15.3.4.1b of the standard.
	 */
	void pushTelemetryToPacketStore(PacketStore& packetStore, const Message& message, Time::DefaultCUC timestamp);

	/**
	 * Resumes the open retrieval of a single packet store as per 6.15.3.4.3 of the standard: sets the open
	 * retrieval status to "in progress" and releases every packet stored from the open retrieval start time tag
	 * onwards. The start time tag is then advanced past the released packets, so that consecutive suspend and
	 * resume operations neither skip nor repeat any packet (requirement 6.15.3.4.1c).
	 *
	 * @param request used to raise errors.
	 * @return true if the open retrieval of the packet store was resumed, false if the instruction was rejected.
	 */
	bool resumeOpenRetrieval(PacketStore& packetStore, const Message& request);
