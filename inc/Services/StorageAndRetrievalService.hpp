...
private:
	 ...
	 */
	static inline String<ECSSPacketStoreIdSize> readPacketStoreId(ECSSMessage& message);
	 ...
	 */
	void copyFromTagToTag(ECSSMessage& request);
	 ...
	 */
	void copyAfterTimeTag(ECSSMessage& request);
	 ...
	 */
	void copyBeforeTimeTag(ECSSMessage& request);
	...
	bool checkPacketStores(const String<ECSSPacketStoreIdSize>& fromPacketStoreId,
	                       const String<ECSSPacketStoreIdSize>& toPacketStoreId, ECSSMessage& request);
	 ...
	 * @param request used to raise errors.
	 */
	static bool checkTimeWindow(uint32_t startTime, uint32_t endTime, ECSSMessage& request);
	 ...
	 * @param request used to raise errors.
	 */
	bool checkDestinationPacketStore(const String<ECSSPacketStoreIdSize>& toPacketStoreId, ECSSMessage& request);
	...
	bool noTimestampInTimeWindow(const String<ECSSPacketStoreIdSize>& fromPacketStoreId, uint32_t timeTag,
	                             ECSSMessage& request, bool isAfterTimeTag);
	 ...
	 * @return true if an error has occurred.
	 */
	bool failedStartOfByTimeRangeRetrieval(const String<ECSSPacketStoreIdSize>& packetStoreId, ECSSMessage& request);

	/**
	 * Forms the content summary of the specified packet-store and appends it to a report message.
	 */
	void createContentSummary(ECSSMessage& report, const String<ECSSPacketStoreIdSize>& packetStoreId);
...
public:
	 ...
	 */
	void executeOnPacketStores(ECSSMessage& request, const std::function<void(PacketStore&)>& function);

	/**
	 * TC[15,1] request to enable the packet stores' storage function
	 */
	void enableStorageFunction(ECSSMessage& request);

	/**
	 * TC[15,2] request to disable the packet stores' storage function
	 */
	void disableStorageFunction(ECSSMessage& request);

	/**
	 * TC[15,9] start the by-time-range retrieval of packet stores
	 */
	void startByTimeRangeRetrieval(ECSSMessage& request);

	/**
	 * TC[15,11] delete the packet store content up to the specified time
	 */
	void deletePacketStoreContent(ECSSMessage& request);
	 ...
	 */
	void packetStoreContentSummaryReport(ECSSMessage& request);

	/**
	 * TC[15,14] change the open retrieval start time tag
	 */
	void changeOpenRetrievalStartTimeTag(ECSSMessage& request);

	/**
	 * TC[15,15] resume the open retrieval of packet stores
	 */
	void resumeOpenRetrievalOfPacketStores(ECSSMessage& request);

	/**
	 * TC[15,16] suspend the open retrieval of packet stores
	 */
	void suspendOpenRetrievalOfPacketStores(ECSSMessage& request);

	/**
	 * TC[15,17] abort the by-time-range retrieval of packet stores
	 */
	void abortByTimeRangeRetrieval(ECSSMessage& request);
	 ...
	 */
	void packetStoresStatusReport(ECSSMessage& request);

	/**
	 * TC[15,20] create packet stores
	 */
	void createPacketStores(ECSSMessage& request);

	/**
	 * TC[15,21] delete packet stores
	 */
	void deletePacketStores(ECSSMessage& request);
	 ...
	 */
	void packetStoreConfigurationReport(ECSSMessage& request);

	/**
	 * TC[15,24] copy the packets contained into a packet store, selected by the time window
	 */
	void copyPacketsInTimeWindow(ECSSMessage& request);

	/**
	 * TC[15,25] resize packet stores
	 */
	void resizePacketStores(ECSSMessage& request);

	/**
	 * TC[15,26] change the packet store type to circular
	 */
	void changeTypeToCircular(ECSSMessage& request);

	/**
	 * TC[15,27] change the packet store type to bounded
	 */
	void changeTypeToBounded(ECSSMessage& request);

	/**
	 * TC[15,28] change the virtual channel used by a packet store
	 */
	void changeVirtualChannel(ECSSMessage& request);
	 ...
	 */
	void execute(ECSSMessage& request);
};
