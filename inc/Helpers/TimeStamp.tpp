...
TimeStamp<secondsCounter, fractionalBytes>::TimeStamp(
	...
	int timestampFractionalBytes = 0;
	int timestampSecondsBytes = 1;
	// int epoch_param = 0;

	if (headerSize == 2) {
		// epoch_param = (timestamp[0] & 0b01110000) >> 4;
		timestampSecondsBytes += (timestamp[0] & SECONDS_FROM_DOUBLE_BYTE_CUC_TIMESTAMP_BITMASK_BYTE1) >> 2;
		timestampSecondsBytes += (timestamp[1] & SECONDS_FROM_DOUBLE_BYTE_CUC_TIMESTAMP_BITMASK_BYTE2) >> 5;
		timestampFractionalBytes =
		...
		// epoch_param = (timestamp[0] & 0b01110000) >> 4;
		timestampSecondsBytes += (timestamp[0] & SECONDS_FROM_SINGLE_BYTE_CUC_TIMESTAMP_BITMASK_BYTE1) >> 2;
		timestampFractionalBytes =
	...
	int err = 0;
	for (int i = headerSize + timestampSecondsBytes + timestampFractionalBytes; i < 9; i++) {
		if (timestamp[i] != 0) {
			err += 1;
			break;
		}
	...
	ASSERT_INTERNAL(timestampSecondsBytes <= secondsCounter,
	                ErrorHandler::InternalErrorType::InvalidTimeStampInput);
	ASSERT_INTERNAL(timestampFractionalBytes <= fractionalBytes,
	                ErrorHandler::InternalErrorType::InvalidTimeStampInput);

	// put timestamp into internal counter
	taiCounter = 0;
	// add seconds until run out of bytes on input array
	for (auto i = 0; i < timestampSecondsBytes; i++) {
		taiCounter = taiCounter << 8;
		taiCounter += timestamp[headerSize + i];
	}
	// add fractional until run out of bytes on input array
	for (auto i = 0; i < timestampFractionalBytes; i++) {
		taiCounter = taiCounter << 8;
		taiCounter += timestamp[headerSize + timestampSecondsBytes + i];
	}
	// pad rightmost bytes to full length
	taiCounter = taiCounter << 8 * (fractionalBytes - timestampFractionalBytes);
}
...
////////////// GETTER ///////////////
template <uint8_t seconds_counter_bytes, uint8_t fractional_counter_bytes>
int TimeStamp<seconds_counter_bytes, fractional_counter_bytes>::asTAIseconds() {
	return taiCounter >> (8 * fractional_counter_bytes);
}

template <uint8_t seconds_counter_bytes, uint8_t fractional_counter_bytes>
const etl::array<uint8_t, Time::CUCTimestampMaximumSize>
TimeStamp<seconds_counter_bytes, fractional_counter_bytes>::toCUCtimestamp() {
  ...
  // cppcheck-suppress redundantCondition
	...
	if (header_size == 1) {
		return_array[0] = static_cast<uint8_t>(CUCHeader);
		index_first_non_header_byte = 1;
	}

	else { // two-bytes CUC header
		return_array[1] = static_cast<uint8_t>(CUCHeader);
		return_array[0] = static_cast<uint8_t>(CUCHeader >> 8);
	...
	for (auto byte_being_filled_in_return = 0;
		...
		return_array[index_first_non_header_byte + byte_being_filled_in_return] =
		    taiCounter >> index_of_interesting_byte_in_tai_counter;
	}
...
template <uint8_t seconds_counter_bytes, uint8_t fractional_counter_bytes>
const UTCTimestamp TimeStamp<seconds_counter_bytes, fractional_counter_bytes>::toUTCtimestamp() {
	using namespace Time;

	int seconds = asTAIseconds();
