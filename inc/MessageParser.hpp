...
private:
	 ...
	 */
	static void parseECSSTCHeader(const uint8_t* data, uint32_t length, Message& message);
	 ...
	 * @param message The Message to modify based on the header
	 */
	static void parseECSSTMHeader(const uint8_t* data, uint32_t length, Message& message);
};
