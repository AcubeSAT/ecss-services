...
namespace Filesystem {
	...
	etl::expected<void, FileReadError> readFile(const Path& path, FileOffset offset, FileDataLength fileDataLength,
		...
		const auto fileSize = fs::file_size(path.data(), errorCode);
		if (errorCode) {
			return etl::unexpected(FileReadError::UnknownError);
		}

		if (static_cast<uint64_t>(offset) + fileDataLength > fileSize) {
		...
		file.seekg(offset);
		file.read(reinterpret_cast<char*>(buffer.data()), fileDataLength); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
		if (not file) {
			return etl::unexpected(FileReadError::ReadError);
		}
	...
	etl::expected<void, FileWriteError> writeFile(const Path& path, FileOffset offset, FileDataLength fileDataLength,
	                                              const etl::span<const uint8_t> buffer) {
		etl::optional<NodeType> nodeType = getNodeType(path);
		if (not nodeType or nodeType.value() != NodeType::File) {
		...
		const auto fileSize = fs::file_size(path.data(), errorCode);
		if (errorCode) {
			return etl::unexpected(FileWriteError::UnknownError);
		}
		...
		std::fstream file(path.data(), std::ios::binary | std::ios::in | std::ios::out);
		if (not file) {
			return etl::unexpected(FileWriteError::WriteError);
		}

		file.seekp(offset);
		file.write(reinterpret_cast<const char*>(buffer.data()), fileDataLength); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
		file.flush();
		if (not file) {
			return etl::unexpected(FileWriteError::WriteError);
		}
