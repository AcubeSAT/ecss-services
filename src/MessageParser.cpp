...
Message MessageParser::parse(uint8_t* data, uint32_t length) {
	...
	const Message::PacketType packetType = ((data[0] & 0x10) == 0) ? Message::TM : Message::TC;
	bool const secondaryHeaderFlag = (data[0] & 0x08U) != 0U;
	const ApplicationProcessId APID = packetHeaderIdentification & static_cast<ApplicationProcessId>(0x07ff);
	auto sequenceFlags = static_cast<uint8_t>(packetSequenceControl >> 14);
	const SequenceCount packetSequenceCount = packetSequenceControl & (~0xc000U); // keep last 14 bits
...
void MessageParser::parseECSSTCHeader(const uint8_t* data, uint16_t length, Message& message) {
	...
	const ServiceTypeNum serviceType = data[1];
	MessageTypeNum const messageType = data[2];
	const SourceId sourceId = (data[3] << 8) + data[4];
...
String<CCSDSMaxMessageSize> MessageParser::composeECSS(const Message& message, uint16_t size) {
	...
	if (message.packetType == Message::TC) {
		...
		uint32_t const ticks = TimeGetter::getCurrentTimeDefaultCUC().formatAsBytes();
...
String<CCSDSMaxMessageSize> MessageParser::compose(const Message& message) {
	...
	const String<CCSDSMaxMessageSize> ecssMessage = MessageParser::composeECSS(message);
	...
	const SequenceCount packetSequenceControl = message.packetSequenceCount | (3U << 14U);
	uint16_t const packetDataLength = ecssMessage.size() - 1;
